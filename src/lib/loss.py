...
class Supervised(LossTerm):

    def __call__(self, net, cfg, extra):
        labels = th.nn.functional.one_hot(net.labels.long(), num_classes=net.output.shape[1]).float()
        return th.nn.functional.binary_cross_entropy(net.output, labels, reduction="mean")
