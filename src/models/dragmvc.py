...
class DRAGMVC(ModelBase):
    def __init__(self, cfg, **kwargs):
        """
        Implementation of the DRAGMVC model.

        :param cfg: Model config. See `config.defaults.DRAGMVC` for documentation on the config object.
        """
        super().__init__()
        ...
        assert all([bb_sizes[0] == s for s in bb_sizes]), f"DRAGMVC requires all backbones to have the same " \
