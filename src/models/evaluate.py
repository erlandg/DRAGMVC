    ...
    with th.no_grad():
        for i, data in enumerate(eval_data):
            ...
            if label.size(0) >= batch_size: 
                batch_losses = net.calc_losses(labels=label.to(config.DEVICE), ignore_in_total=IGNORE_IN_TOTAL)
...
def get_logs(cfg, net, eval_data, iter_losses=None, epoch=None, include_params=True, graph_bool=False, return_pred_labels=False):
    ...
    if (epoch is None) or ((epoch % cfg.eval_interval) == 0) or (epoch == 1):
        ...
        pred = helpers.correct_predictions(pred, *helpers.ordered_cmat(labels, pred, return_ri_ci = True)[-1])
    else:
        pred = labels = None
    if include_params:
