...
def wandb_init(name, cfg):
    ...
    parameters = {
        ...
        "graph_mask_weighting": mask_weighting,
...
def train(cfg, net, loader, eval_data = None, graph_bool=False, callbacks=tuple(), track_logs=False):
    ...
    while e <= cfg.n_epochs:
        ...
        logs, (eval_pred, eval_labels) = evaluate.get_logs(
            cfg,
            net,
        ...
        if track_logs:
            wand_logs = logs
            if (eval_pred is not None) and (eval_labels is not None):
                if len(config.LABELS_STRING.values()) == len(set(eval_labels)):
                    cn = list(config.LABELS_STRING.values())
                else:
                    cn = None
                try:
                    wand_logs["metrics/cmat"] = wandb.plot.confusion_matrix(
                        y_true = eval_labels,
                        preds = eval_pred,
                        class_names = cn
                    )
        ...
        if not graph_bool:
            ...
            if cfg.batch_size == len(dataset.tensors[-1]):
                eval_data = get_graph_sampler(
                    ...
                    equal_split = cfg.model_config.loss_config.n_semi_supervised
                )
