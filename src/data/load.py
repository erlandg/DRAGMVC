...
class GraphSampler(GraphSAINTSampler):
    ...
    def __init__(self, data, batch_size: int, walk_length: int,
                 ...
                 semi_supervised_N: int = None, equal_split: bool = False,
                 **kwargs):
        self.labels = data.y
    ...
    def update_index(self):
        ...
        if self.semi_supervised_N is not None:
            if self.equal_split:
                N_equal = self.semi_supervised_N // self.num_classes
                semi_supervised_idx = np.concatenate(tuple(np.random.choice(
                    np.argwhere(self.labels == label)[0],
                    size = N_equal,
                    replace = False
                ) for label in np.unique(self.labels)))
                diff = self.semi_supervised_N - (N_equal * self.num_classes)
                if diff > 0:
                    semi_supervised_idx = np.concatenate((
                        semi_supervised_idx,
                        np.random.choice(
                            list(set(range(len(self.labels))) - set(semi_supervised_idx)),
                            size = diff,
                            replace = False
                        )
                    ))
            elif not self.equal_split:
                semi_supervised_idx = np.random.choice(
                    range(len(self.labels)),
                    size = self.semi_supervised_N,
                    replace = False
                )
